...
static const bool8 sAllowedNationalDexNumbers[NATIONAL_DEX_COUNT + 1] =
{
    [1] = TRUE,
    [2] = TRUE,
    [3] = TRUE,
...
bool8 IsSpeciesAllowedByCustomList(u16 species, u8 formId)
{
    ...
    u16 formSpecies = (formId == 0 && species > FORMS_START) ? species : GetFormSpeciesId(species, formId);
    u16 nationalDexNum = SpeciesToNationalPokedexNum(baseSpecies);

    if (nationalDexNum == 0 || nationalDexNum > NATIONAL_DEX_COUNT)
        return FALSE;

    if (!sAllowedNationalDexNumbers[nationalDexNum])
        return FALSE;
...
u16 GetEvolutionTargetSpecies(struct Pokemon *mon, u8 type, u16 evolutionItem, u16 tradePartnerSpecies, u8 *targetFormId)
{
    ...
    *targetFormId = 0;
    ...
    if (targetSpecies != SPECIES_NONE && !IsSpeciesAllowedByCustomList(targetSpecies, *targetFormId))
        return SPECIES_NONE;
